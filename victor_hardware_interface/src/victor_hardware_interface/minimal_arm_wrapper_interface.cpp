...
namespace victor_hardware_interface {
    ...
    bool
    cartesianPexpEqual(const CartesianPathExecutionParameters &pexp1, const CartesianPathExecutionParameters &pexp2) {
    ...
    bool controlModeParamsEqual(const ControlModeParameters &params1, const ControlModeParameters &params2) {
        ...
        const bool jpexp_equal = jointPexpEqual(params1.joint_path_execution_params,
                                                params2.joint_path_execution_params);
        const bool cpexp_equal = cartesianPexpEqual(params1.cartesian_path_execution_params,
                                                    params2.cartesian_path_execution_params);

        // Joint Impedance mode parameters
        const bool jd_equal = jvqEqual(params1.joint_impedance_params.joint_damping,
                                       params2.joint_impedance_params.joint_damping);
        const bool js_equal = jvqEqual(params1.joint_impedance_params.joint_stiffness,
                                       params2.joint_impedance_params.joint_stiffness);
        ...
        const bool nd_equal = (params1.cartesian_impedance_params.nullspace_damping ==
                               params2.cartesian_impedance_params.nullspace_damping);
    ...
    MinimalArmWrapperInterface::MinimalArmWrapperInterface(
            ...
            : nh_(nh), cartesian_control_frame_(cartesian_control_frame),
              set_control_mode_timeout_(set_control_mode_timeout), send_lcm_ptr_(send_lcm_ptr),
        ...
        motion_command_sub_ = nh_.subscribe(motion_command_topic, 1,
                                            &MinimalArmWrapperInterface::motionCommandROSCallback, this);
        gripper_command_sub_ = nh_.subscribe(gripper_command_topic, 1,
                                             &MinimalArmWrapperInterface::gripperCommandROSCallback, this);;
        set_control_mode_server_ = nh_.advertiseService(set_control_mode_service,
                                                        &MinimalArmWrapperInterface::setControlModeCallback, this);
        get_control_mode_server_ = nh_.advertiseService(get_control_mode_service,
                                                        &MinimalArmWrapperInterface::getControlModeCallback, this);
    }
    ...
    std::pair<bool, std::string> MinimalArmWrapperInterface::validateControlMode(const ControlModeParameters &params) {
        ...
        const auto valid_cartesian_impedance_params = validateCartesianImpedanceParams(
                params.cartesian_impedance_params);
        const auto valid_cartesian_control_mode_limits = validateCartesianControlModeLimits(
                params.cartesian_control_mode_limits);
        const auto valid_joint_path_execution_params = validateJointPathExecutionParams(
                params.joint_path_execution_params);
        const auto valid_cartesian_path_execution_params = validateCartesianPathExecutionParams(
                params.cartesian_path_execution_params);
    ...
    bool MinimalArmWrapperInterface::setControlModeCallback(
        ...
        if (local_active_control_mode_copy.Valid()) {
            const auto merged_command = mergeControlModeParameters(local_active_control_mode_copy.GetImmutable(),
                                                                   req.new_control_mode);
    ...
    std::pair<bool, std::string> MinimalArmWrapperInterface::validateCartesianPose(
        ...
        if (frame != cartesian_control_frame_) {
            valid = false;
            message +=
                    "+Commanded cartesian pose has the wrong frame, " + frame + " given, " + cartesian_control_frame_ +
                    " expected";
        }
            ...
            switch (command_motion_mode) {
                ...
                default:
                    return std::make_pair(false,
                                          std::string("Invalid commanded control mode. This should not be possible"));;
            }
        } else {
